  ...
  const fetchEventDetails = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        setAttachments(eventData.attachments?.map(name => ({ name })) || []);
        ...
        reset({
          ...
          registrationDeadline: eventData.registrationDeadline ? eventData.registrationDeadline.replace('Z', '') : '',
      ...
      <div className="max-w-4xl mx-auto">
        ...
        >
          <div className="flex items-center justify-between mb-8">
            <h1 className="text-3xl font-bold text-white">Edit Event: {event.title}</h1>
            <button
